...
function pas_metabox_function( $post ) {
	...
	$pas_metabox_status = get_post_meta( $post->ID, '_pas_metabox_status', true );
	...
	switch ( $pas_metabox_status ) {
		case '':
			...
			add_post_meta( $post->ID, '_pas_metabox_status', 'active', true );
...
function pas_metabox_save_meta( $post_id ) {
	...
	if ( isset( $_POST['pas_metabox_status'] ) ) {
		...
		update_post_meta( $post_id, '_pas_metabox_status', strip_tags( $_POST['pas_metabox_status'] ) );
	}
...
function pas_self_manage_get_task_posts( $query ) {
	...
	if ( is_page('Tasks') && $query->is_main_query() ) //*** need to be modified to a variable
		$query->set( 'posts_per_page', 10 );
		//$query->set( 'post_type', array( 'post', 'to_do_list' ) );
...
function pas_change_task_status_form($content) {
	...
	if ( isset( $_POST['pas_metabox_status'] ) ) {
		...
		//save the metadata
		update_post_meta( $current_post_id, '_pas_metabox_status', strip_tags( $_POST['pas_metabox_status'] ) );
	}
